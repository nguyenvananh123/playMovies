<template>
  ...
  <div>
    <iframe
      ...
      :src="`${episodeLinks ? episodeLinks.link : ''}`"
      frameborder="0"
    >
    </iframe>
  </div>
...
onBeforeMount(async () => {
  const movieId = route.params.id;
  episodes.value = await listEpisode.getEpisode(movieId);
  const episodeID = episodes.value.find((episode) => episode.id);
  if (!episodeID.id) return;
  episodeLinks.value = await episodeLink.getEpisode(episodeID.id);
});
